...
import com.pim.foreignexchange.config.KafkaProperties;
...
class ForeignExchangeKafkaResourceIT {
    ...
    void setup() {
        ...
        ForeignExchangeKafkaResource kafkaResource = new ForeignExchangeKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/foreign-exchange-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/foreign-exchange-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
